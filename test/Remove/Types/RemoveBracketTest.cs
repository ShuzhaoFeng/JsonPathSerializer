...
public class RemoveBracketTest
{
    ...
    [TestMethod]
    public void ThrowsExceptionWhenRemovingPropertyWithStringInBracketWithoutQuote()
    {
        Assert.ThrowsException<JsonException>(() => _loadedManager.Remove("name[last]"));
    }

    [TestMethod]
    public void ThrowsExceptionWhenRemovingPropertyWithUnclosedBracket()
    {
        Assert.ThrowsException<JsonException>(() => _loadedManager.Remove("name['last'"));
    }

    [TestMethod]
    public void ThrowsExceptionWhenRemovingPropertyWithUnopenedBracket()
    {
        Assert.ThrowsException<JsonException>(() => _loadedManager.Remove("name'last']"));
    }
}